...
def main():
    ...
    for seed in args.seed_list:
        ...
        for model in tqdm(range(num_models)):
            ...
            if args.filt_prompts_catg_list!=0: auroc_by_layer.append(roc_auc_score(labels, [-v for v in np.squeeze(test_preds[model])]) if ('knn' in args.probes_file_name) or ('kmeans' in args.probes_file_name) else roc_auc_score(labels, np.squeeze(test_preds[model])))
        ...
        if len(labels)>0:
            ...
            if args.filt_prompts_catg_list!=0: seed_results_list.append(roc_auc_score(labels, [-v for v in np.squeeze(all_preds[num_layers-1])]) if ('knn' in args.probes_file_name) or ('kmeans' in args.probes_file_name) else roc_auc_score(labels,np.squeeze(all_preds[num_layers-1]))) # print(roc_auc_score(labels,np.squeeze(all_preds[num_layers-1,:,:])))
            ########################
