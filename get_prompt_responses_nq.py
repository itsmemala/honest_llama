...
def main(): 
    ...
    labels = []
    for i,batch in enumerate(list(dataset.take(3610))): # one row at a time
        labels_dict['exact_match'] = 0.0
        reference_answers = batch['answer']
        for answer in reference_answers:
            predictions = [responses[i].lstrip()]
            references = [answer]
            results = exact_match_metric.compute(predictions=predictions,
                                                    references=references,
                                                    ignore_case=True,
                                                    ignore_punctuation=True)
            sequence_dict['exact_match'] = max(results['exact_match'], labels_dict['exact_match'])
            rouge_results = rouge.compute(predictions=predictions, references=references)
            for rouge_type in rouge_types:
                labels_dict[rouge_type + '_to_target'] = max(rouge_results[rouge_type].mid.fmeasure,
