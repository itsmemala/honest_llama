...
def main():
    ...
    if args.layer_ends is None:
        ...
        all_val_loss = np.concatenate([np.load(f'{args.save_path}/probes/{args.results_file_name}_{layer_start}_{layer_end}_val_loss.npy') for layer_start,layer_end in zip(args.layer_starts,args.layer_ends)], axis=1)
        all_train_loss = np.concatenate([np.load(f'{args.save_path}/probes/{args.results_file_name}_{layer_start}_{layer_end}_train_loss.npy') for layer_start,layer_end in zip(args.layer_starts,args.layer_ends)], axis=1)
