    ...
    for fold in range(len(all_test_f1s)):
        ...
        if 'ah' in args.results_file_name:
            ...
            for model in range(len(all_val_loss[fold])):
                print('Val loss model',model,':',all_val_loss[fold][model],'Val F1:',"{:.2f}".format(all_val_f1s[fold][model]),"{:.2f}".format(val_f1_cls0[model]),'Test F1:',"{:.2f}".format(all_test_f1s[fold][model]))
        print('\n')
