...
def get_probe_wgts(fold,model,results_file_name,save_path,args):
    ...
    kld_probe = 0
    # linear_model = torch.load(f'{save_path}/probes/models/{results_file_name}_model{fold}_{layer}_{head}_{kld_probe}')
...
def main():
    ...
    for fold in range(len(all_test_f1s)):
        ...
        print('PCA:')
        pca = PCA(n_components=25) # KernelPCA(n_components=100, kernel='poly') # PCA(n_components=3)
