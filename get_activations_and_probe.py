                ...
                if args.method=='individual_linear':
                    ...
                    with torch.no_grad():
                        ...
                        for step,batch in enumerate(ds_val):
                            ...
                            predicted = torch.max(linear_model(inputs).data, axis=1)[1] if args.token in ['answer_last','prompt_last','maxpool_all'] else torch.stack([torch.max(torch.max(linear_model(inp).data, axis=0)[0], axis=1)[1] for inp in inputs]) # For each sample, get max prob per class across tokens, then choose the class with highest prob
                    ...
                    with torch.no_grad():
                        ...
                        for step,batch in enumerate(ds_test):
                            ...
                            test_preds += torch.max(linear_model(inputs).data, axis=1)[0] if args.token in ['answer_last','prompt_last','maxpool_all'] else torch.stack([torch.max(linear_model(inp).data, axis=0)[0] for inp in inputs]) # For each sample, get max prob per class across tokens
