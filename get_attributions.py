...
def main(): 
    ...
    for start, end in load_ranges:
        ...
        for i,(row,tokenized_prompt,tokens,answer_token_idx) in tqdm(enumerate(zip(data[start:end],tokenized_prompts[start:end],prompt_tokens[start:end],answer_token_idxes[start:end]))):
            if args.mlp_l1=='Yes':
